...
//.map() method to creat new array
 ...
 //definng Array
const words:string[] = ["Governor", "Sindh", "IT", "Course"];
// .map() to creat new array
const lengths: number[] = words.map(word => word.length);// gives new array
...
const numbersArray: number[] = [5, 8, 11, 17,20,25,32];//Array of numbers
...
function CurrentDateFormate(): string {
    const date = new Date();
...
let num1 = -5
let num2 = 5
const difference: number = Math.abs(num1-num2);
